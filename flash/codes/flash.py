...
def flash():
    ...
    # pre_numSolutions = int(8 * math.log(t/delta))
    ...
    pre_numSolutions = int(8 * math.log(2 * t/delta))

    lo = (1+epsilon)/(1-epsilon)
    hi = 1 + (eta+9*epsilon)/4
    ...
    tempFile = inputFile[:-6] + "_t.cnf"    # contains Kernel-output formula
    ...
    while i < t:
        ...
        samplerSample = getSolutionFromSampler(inputFile = inputFile,
                                            ...
                                            indVarList = indVarList,
                                            seed = seed)[0]
        projectedsamplerSample = project(samplerSample, UserIndVarList)
        projectedidealSample = project(idealSample, UserIndVarList)
        # projectedsamplerSample = samplerSample
        # projectedidealSample = idealSample

        # idealSample = projectedidealSample

        print("indVarList: {0}, [len: {1}]".format(indVarList, len(indVarList)))
        print("userIndVarList: {0}, [len: {1}]".format(UserIndVarList, len(UserIndVarList)))

        print("projectedsamplersample (sigma1):", projectedsamplerSample)
        print("projectedidealsample (sigma2):", projectedidealSample)

        # loop out?
        if set(projectedsamplerSample) == set(projectedidealSample):
            print("no two different witnesses found")
            continue

        # with the assumption that the variable ordering is same
        curTilt = tilt(weight_map, projectedsamplerSample, projectedidealSample, UserIndVarList)
        # curTilt = max(curTilt, 1.0/curTilt)

        print("curTilt (sigma1 / sigma2) :", curTilt)
        L = (curTilt*lo)/(1+curTilt*lo)
        H = (curTilt*hi)/(1+curTilt*hi)
        T = (H+L)/2
        f.write("Threshold T : {0}\t Tilt : {1}\n".format(T, curTilt))

        factor1, factor2 = weightFactor(weight_map, projectedsamplerSample, projectedidealSample, UserIndVarList, epsilon)
        print("factors--", factor1, factor2)
        numSolutions = int(pre_numSolutions*H/(H-L)**2)
        # MaxSolutions = 2 * numSolutions
        # MaxSolutions = int(max(numSolutions + math.log(t/delta), factor * numSolutions))
        MaxSolutions = int((math.sqrt(math.log(2*t / delta)) + math.sqrt(math.log(2*t / delta) + 4 * numSolutions * factor1)) / (2 * factor2))**2
        ...
        f.write("numSolutions : {0}\t MaxSolutions : {1}\n".format(numSolutions, MaxSolutions))
        ...
        tempIndVarList = constructKernel(inputFile = UserInputFile, #inputFile
                                         tempFile = tempFile,
                                         samplerSample = projectedsamplerSample,
                                         idealSample = projectedidealSample,
                                         numSolutions = MaxSolutions,
                                         origIndVarList = UserIndVarList) #indVarList
        print("isHorn:",check_cnf(tempFile))
        samplinglist = list(chainform.Transform(tempFile, tempFile, 4))  # precision set to 4
        # samplinglist = indVarList
        print("file was constructed with these", projectedsamplerSample, projectedidealSample)
        print("tempIndVarList:", tempIndVarList, "\nsamplinglist:", samplinglist)
        f.write("file was constructed with these {0}, {1} \n".format(str(projectedsamplerSample), str(projectedidealSample)))
        ...
        for iter in range(int(MaxSolutions / hop) + 1):
            ...
            sampled += hop
            seed += 1
        bias, n0 = biasFind(projectedsamplerSample, projectedidealSample, solList, UserIndVarList, f)
